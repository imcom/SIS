	...
	class HttpHandler implements Runnable{
		...
		public void run(){
			try {
				...
				byte[] t_response = null;
				try {
					int status = client.executeMethod(getMethod);
					if(status != HttpStatus.SC_OK){
						System.out.println("Request failed!");
					}else{
						System.out.println(getMethod.getStatusCode() + "Status: " + getMethod.getStatusText());
						t_response = getMethod.getResponseBody();
					}
					...
					try {
						response = new String(t_response, getMethod.getResponseCharSet());
					...
					byte[] snd_array = response.getBytes();
					int i, offset = 1024;
					...
					if(i < snd_array.length){
						//System.out.println("new String :"+new String(snd_array,i,snd_array.length-i));
